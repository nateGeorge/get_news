...
--> 417     continually_scrape_rss()
    418
    419 # to convert timezone

~/github/get_news/scrape_reuters_rss.py in continually_scrape_rss()
     75             print(l)
     76             feeds = feedparser.parse(f)
---> 77             if feeds['status'] != 200:
     78                 print('status is not good:', str(feeds['status']) + '; exiting')
     79                 break
